...
// CSS MODERNE BASÉ SUR LE DESIGN DE LA PAGE D'ACCUEIL
const modernHeroCSS = `/* ===== HERO MODERNE - DESIGN PAGE D'ACCUEIL APPLIQUÉ ===== */
        .hero-sport,
        .hero-faq,
        .hero-livraison,
        .hero-contact,
...
console.log('🚀 Application du design de la page d\'accueil à TOUTES les pages...\n');
...
productPages.forEach(pagePath => {
    ...
    if (styleMatch) {
        ...
        // Supprimer tous les anciens CSS de hero (toutes les variantes)
        styleContent = styleContent.replace(/\/\*[\s\S]*?HERO[\s\S]*?\*\/[\s\S]*?(@media \(max-width:)/g, '@media (max-width:');
        styleContent = styleContent.replace(/\.hero-sport[\s\S]*?(?=(@media \(max-width:|<\/style>))/g, '');
        styleContent = styleContent.replace(/\.hero-faq[\s\S]*?(?=(@media \(max-width:|<\/style>))/g, '');
        styleContent = styleContent.replace(/\.feature-badge[\s\S]*?(?=(@media \(max-width:|<\/style>))/g, '');
...
infoPages.forEach(pagePath => {
    ...
    if (content.includes('<style>')) {
        ...
        if (styleMatch) {
            let styleContent = styleMatch[1];
            styleContent = styleContent.replace(/\/\*[\s\S]*?HERO[\s\S]*?\*\/[\s\S]*?(@media \(max-width:|<\/style>)/g, '$1');
            const newStyleBlock = styleContent + '\n' + modernHeroCSS + '\n    ' + styleMatch[2];
            content = content.replace(styleRegex, newStyleBlock);
        }
    } else {
...
console.log(`\n🎉 Terminé ! ${modifiedCount} pages avec le design de la page d'accueil.`);
console.log('✨ Design identique partout : gradient clair, formes animées, boutons modernes !');
