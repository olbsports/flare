...
// CSS propre et complet de la page d'accueil
const cleanHeroCSS = `/* ===== HERO MODERNE - DESIGN IDENTIQUE PAGE D'ACCUEIL ===== */
        .hero-sport,
        .hero-faq,
        .hero-livraison,
        .hero-contact,
...
console.log('🔧 Nettoyage et réparation de TOUTES les pages...\n');

let fixedCount = 0;
let errors = [];

function cleanAndFixCSS(filePath) {
    try {
        let content = fs.readFileSync(filePath, 'utf-8');

        // Vérifier si la page a un hero
        ...
        // Tout ce qui est AVANT le <style>
        const beforeStyle = content.substring(0, styleStartIndex);

        // Tout ce qui est APRÈS le </style>
        const afterStyle = content.substring(styleEndIndex + 8);

        // Reconstruire avec le CSS propre
        const newContent = beforeStyle + '<style>\n' + cleanHeroCSS + '\n    </style>' + afterStyle;
...
console.log(`\n🎉 ${fixedCount} pages réparées et nettoyées !`);
